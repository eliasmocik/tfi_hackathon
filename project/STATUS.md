...
- **WP-C** `src/wp_c_burden.py` — implementability sweep. C5 as stated is
  falsified: k = 2 daily retains 54.1 % of the ideal gain, k = 3 only 43.7 %.
  The partition design matters more than k.
- **WP-H** `src/wp_h_pack.py` → `out/EVIDENCE_PACK.md`, consultation-shaped.
- **WP-F** not run — see `out/WPF_NOT_RUN.md`. The 15 real CG3 units cannot be
  matched to the gss1 priority register (0 of 15 by name), so the NPD-first
  scenario stays simulated (rule 1N), not measured.
- **WP-D** not run — the euro, carbon and CBF inputs are all still open in the
  brief's verification queue, so `wpg_mcda` sweeps the rate instead of
  asserting one. WP-I is out of scope by the brief's own cut order.
